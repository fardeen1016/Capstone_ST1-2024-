...
class InsuranceApp:
    ...
    def setup_gui(self):
        predicted_price_button = ttk.Button(
            self.master, text="1. Predicted Price", command=self.show_predicted_price
        )
        predicted_price_button.pack(fill=tk.X, padx=5, pady=5)
        dataset_button = ttk.Button(
            self.master, text="2. Insurance Dataset", command=self.show_dataset
        )
